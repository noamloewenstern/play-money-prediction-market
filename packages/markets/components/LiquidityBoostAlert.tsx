...
export function LiquidityBoostAlert({ onClick }: { onClick: () => void }) {
  ...
  return (
    <Alert className={cn('[&>svg]:text-primary-foreground', classNames)} onMouseMove={handleMouseMove}>
      <DiamondPlus className="h-4 w-4" />
      <AlertDescription className="text-muted-foreground">
        Boost the liquidity on this market to earn trader bonuses, and more!
      </AlertDescription>
      <AlertTitle className="mb-0">
        <Button variant="link" className="h-auto p-0 text-primary-foreground underline" onClick={onClick}>
          Add Liquidity Boost
